...
## 8. Future CPS ASEC updates: what moves, and where it lands

The pool is a function of the CPS ASEC, so every change to the ASEC reaches it.
Four kinds of change, in the order they will arrive, and which stage of the build
absorbs each. The build is year-general (S18(c)), so a new year is a re-run, not
a redesign; the other three are not.

**(1) A new survey year (income year 2025 arrives with ASEC 2026).** Extend
`common_ipums_download`'s `parameters.cps.yaml` (the shared extract; variable
additions are made there, never here), re-pull, run `01`–`05` for the new year,
then `13`–`15` to extend the filer counts, the S18(b) series and the handoff
partition. The ceiling moves only when the *anchor* inputs move: Pub 1304 Table
1.6 and HT2 for that year (both reached TY2023 in 2026). Until they do, the new
ASEC year is buildable but not calibratable, and the pool stays at the last
anchored year.

**(2) The corrected ASEC (`CPS-ASEC-Corrected`, in research phase 2026-09).**
It will ship as a *merge file* keyed on `YEAR`, `SERIAL`, `PERNUM` with corrected
transfer, retirement and investment income beside the originals, a reweighted
universe (whole-supplement non-respondents dropped, then a response-propensity
adjustment and rake), and a draw structure for uncertainty. The pool builder
keeps `SERIAL` and `PERNUM` on its person table, so the join is available
without redesign. What it would change, and where:

| correction | reaches the pool through | expected direction |
|---|---|---|
| drop-and-reweight of whole-supplement imputes (15–20% of persons, income fully hot-decked) | the *universe* `01_build_units.R` starts from and the weight it carries; the filing model then scores fewer noise-classified units | composition within band sharpens; the level is unaffected because calibration hits the administrative residual regardless |
| interest and dividends re-targeted to the national-accounts concept | `add_asec_income_concepts()` inputs; Pub 5785 receipt margins in `04_calibrate.R` | non-filer interest falls (2023 pool carries $16.8B; the survey exceeds the tax-return benchmark and ~80% of interest dollars are imputed) |
| retirement receipt under 65 calibrated to Brady–Bass moments | gross income in the filing-requirement test, hence who is above threshold | some units now below threshold move above it; the above-threshold hazard level (Pub 5785) is external, so the effect is on *which* units, not how many |
| unemployment insurance (LMS method; 2020 capture ≈40% of dollars) | the same threshold test, 2020–21 only | matters for the S20 years; direction as above |
| OASDI receipt (−15% recipients in linked data) | `INCSS`; the Social Security acceptance dimension in `06_acceptance.R` | more non-filers with benefits; taxation-of-benefits exposure rises |

Two constraints carry over from that repo's decisions: the corrected file is
**national-only** (its D9) while the pool feeds state margins, so the state work
would stay on the uncorrected weights or re-discipline the correction at the
state level; and the original values are retained beside corrections (its D3),
so the pool can be built on either and the difference measured. **The right
first step, before any adoption:** measure the share of emitted non-filer weight
that comes from whole-supplement-imputed persons, using `UH_SUPREC_A2`, which
is already in the common extract. That number says how much (2) can move.

**(3) A processing-system or questionnaire break, like 2019.** The 2019 change
split retirement income into five components (S16 bridges it) and changed the
imputation model and allocation flags. A future break of that kind is handled
where S16 handled the last one — in `add_asec_income_concepts()` — with the
bridge recorded as a decision and the affected years' gates re-run and compared.
The ASEC modernisation Census has announced is the one on the horizon; its
income-item changes will need a new bridge, not a new method.

**(4) Weight revisions.** Census re-benchmarks ASEC weights to new population
controls (the 2020-Census-based controls landed in the 2022 file). Because the
pool is calibrated to the administrative residual per band, a level shift in
`ASECWT` is absorbed by the calibration; what moves is the within-band
composition and the group-quarters share, both of which `unit_gates_{year}.csv`
and `gq_backfill_summary_{year}.csv` record, so a re-run shows the effect
directly. The pandemic weights (`ASECWTCVD`, 2019–2021) are the one case where
an alternative weight exists: the corrected-file project carries it as a column
rather than the base, and this build does the same.

## Revision history

- **2026-09-11** — moved to Tax-Data with the rest of the workstream; §8 added
  on how future CPS ASEC changes (new years, the corrected file, processing
  breaks, weight revisions) reach the pool and where each is absorbed.
