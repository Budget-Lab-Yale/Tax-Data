...
**Home of the builder: Tax-Data, as of 2026-09-11 (JI).** The paragraph
below recorded the earlier "eventually, not yet" and its reason — the shared
machinery with the state work. That reason dissolved when the state-weights
work moved too: the whole workstream now lives in Tax-Data (`research/
state_weights/`, modules in `src/nonfilers/`); Tax-Simulator keeps the state
tax law and consumes weights through the Tax-Data interface. Kept for the
record:
