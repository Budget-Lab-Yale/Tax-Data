...
CBO_SSAREA <- file.path(raw_data_root(),
...
#-------------------------------------------------------------------------------
# The PRE-SCALE emit, rebuilt here from the calibrated units and the GQ persons
# exactly as 05_emit_pool.R builds it. Not read from nonfiler_pool_{T}.csv.gz:
# since Block D that file carries the S19 scale this script derives, so reading
# it back made the script circular -- a re-run after 05 returned emitted =
# target and scale = 1 (found 2026-09-11 when the migrated copy was gated
# against the committed table). Same filters as 05: nondependent heads 18+,
# expected weight > 0; GQ persons with a scored p_file below 1 and age 18+.
st  <- readRDS(file.path(RES, sprintf('units_%d.rds', T_YEAR)))
u   <- readRDS(file.path(RES, sprintf('calibrated_units_%d.rds', T_YEAR)))
gqp <- fread(file.path(RES, sprintf('gq_persons_%d.csv.gz', T_YEAR)))
