...
# stage3_prototype.R
#
# Stage 3 Phase 3a prototype: exponential
# tilting at donor selection, per income
# decile, hitting SCF moments for
#   {net_worth, cash, equities}
#   x {total $, count > 0}.
#
# Design memo: stage3_calibration_design.md.
#
# Stage 2 context: large spec, num.features=50,
# TRAIN_SEED=1337, DRF cache
# resources/cache/qrf/wealth_drf_ablation_large_t1337_f50.rds.
#
# Per decile (10 cells): solve 6-dim
# lambda via convex Newton on max-ent
# dual. Redraw donors under tilted
# probabilities. Compare pre/post to SCF
# targets; acceptance tolerance 3%
# relative.
#
# Output: resources/cache/stage3_prototype.rds
#         plus tabular report on stdout.
#--------------------------------------
...
solve_tilt = function(records, weights, leaf_donors, F_mat, T_target,
                      max_iter = 40, tol_rel = 1e-5,
                      max_bt = 25, verbose = FALSE) {
  n = length(records)
  k = ncol(F_mat)
  ...
  F_all    = F_mat[all_d, , drop = FALSE]
  w_expand = rep(weights, times = ls)
  M        = length(rec_idx)
  # Target scale for relative-gradient convergence.
  t_scale  = pmax(abs(T_target), 1)
  ...
  eval_fit = function(lam) {
    ...
    list(p_all = p_all, achieved = ach,
         grad = ach - T_target,
         rel = max(abs(ach - T_target) / t_scale))
  }
  ...
  for (iter in seq_len(max_iter)) {
    ...
    # Hessian = H1 - H2.
    # H1 = sum_r wp_r F_r F_r^T = crossprod(sqrt(wp) * F_all)
    F_wp = F_all * sqrt(wp)
    H1 = crossprod(F_wp)

    # H2 = sum_i w_i E_i[f] E_i[f]^T where E_i[f_k] = sum_{j in L_i} p_ij f_k(Y_j)
    ...
    H = H1 - H2
    # Small regularization (in units of H's diagonal) to keep the solve
    # stable when a target dimension has nearly-zero within-leaf variance.
    reg = 1e-8 * max(1, max(abs(diag(H))))
    H_reg = H + diag(reg, k)
    ...
    if (!improved) {
      if (verbose)
        cat(sprintf('  iter %d: backtrack exhausted, stopping\n', iter))
      break
    }
    lambda = lam_try
    cur = fit_try
...
write_rds(out, 'resources/cache/stage3_prototype.rds')
cat('\nwrote resources/cache/stage3_prototype.rds\n')
cat('Done.\n')
